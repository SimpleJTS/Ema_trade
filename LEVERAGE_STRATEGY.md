...
**结果**: 使用 **9x** 杠杆（趋势强度略有提升，但被高波动限制）⚠️
...
| 杠杆 | 触发条件 | 出现概率 |
|------|---------|---------|
| **20x** | ATR<70% + ADX>35 (8×1.5×1.3=15.6→15或更高) | 罕见 |
| **15x** | ATR<70% + ADX>35 | 少见 |
| **13x** | ATR<70% + ADX 25-35 | 偶尔 |
| **12x** | ATR<70% + ADX 25-35 | 偶尔 |
| **10-11x** | ATR 70-120% + ADX>35 | 常见 |
| **9x** | ATR 70-120% + ADX 25-35 | 常见 |
| **8x** | 高波动 或 弱趋势 | 最常见 |

**预期**：
- 60-70% 的时间使用 8-9x 杠杆（安全）
- 20-30% 的时间使用 10-12x 杠杆（适中）
- 5-10% 的时间使用 13-15x 杠杆（激进）
- <5% 的时间使用 >15x 杠杆（极少）
...
- ✅ 仍然可以交易
- ⚠️ 杠杆最高只能到 8x × 1.0 × 1.3 = 10.4x（如果ADX>35）
- ⚠️ 通常保持在8-9x
...
```
[BTCUSDT] 动态杠杆: 12x (基础8x, 低波动(65.5%<70%), 强趋势(ADX=28.3≥25) → 系数1.5×1.15=1.73)
[ETHUSDT] 动态杠杆: 15x (基础8x, 低波动(58.2%<70%), 超强趋势(ADX=38.5>35) → 系数1.5×1.3=1.95)
[SOLUSDT] 动态杠杆: 8x (基础8x, 高波动(185.5%>120%), 强趋势(ADX=31.2≥25) → 系数1.0×1.15=1.15)
[MEMEFIUSDT] 动态杠杆: 9x (基础8x, 高波动(2000.0%>120%), 强趋势(ADX=28.0≥25) → 系数1.0×1.15=1.15)
```
